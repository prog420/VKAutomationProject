...
@allure.epic("API")
@allure.feature("/api/user/<username>/block - Block User")
class TestBlockUserAPI(BaseAPICase):
    @pytest.mark.API
    @pytest.mark.parametrize(
        "title, test_case_link, user, method, expected_status",
        [test_case["case"] for test_case in APIData.block_user_data],
        ids=[test_case["case"][0] for test_case in APIData.block_user_data]
    )
    def test_block_user(self, title, test_case_link, user, method, expected_status):
        """
        Parametrized test if user can be blocked
        ...
        response = self.api_client.block_user(
            method=method, path=f"/api/user/{user['username']}/block",
            expected_status=expected_status, repeat=user.get("repeat", 1)
        )
        # Check value of "access" field in database if expected status is 200
        if expected_status == 200:
            assert response.json()['status'] == 'ok', \
                f"Expected 'ok' status, got '{response.json()['status']}'"
            user_mysql = self.sql_client.get_user(username=user["username"])
            assert user_mysql.access == 0
